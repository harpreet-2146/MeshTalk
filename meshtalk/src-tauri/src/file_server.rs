...
impl FileServer {
    pub fn new() -> Self {
        ...
        let primary = base_dir.join("MeshTalk").join("shared_files");
        let legacy = base_dir.join("Pingo").join("shared_files");
        let storage_dir = if primary.exists() {
            primary
    ...
    pub fn start(&self, preferred_port: u16) -> Result<u16, String> {
        ...
        let server = match tiny_http::Server::http(format!("0.0.0.0:{}", preferred_port)) {
            Ok(s) => {
                println!(
                    "[MeshTalk] File server bound to preferred port {}",
                    preferred_port
                );
                s
            }
            Err(e) => {
                println!(
                    "[MeshTalk] Failed to bind to port {}: {}. Trying random port...",
                    preferred_port, e
                );
        ...
        *self.port.write().unwrap() = actual_port;
        println!(
            "[MeshTalk] File server listening on port {} and ready",
            actual_port
        );
        ...
        thread::spawn(move || {
            println!("[MeshTalk] File server request handler thread started");
            for request in server.incoming_requests() {
                ...
                if let Some(file_id) = url.strip_prefix("/file/") {
                    ...
                    let resp =
                        tiny_http::Response::from_string("MeshTalk File Server").with_header(cors());
                    let _ = request.respond(resp);
                }
            }
        });
