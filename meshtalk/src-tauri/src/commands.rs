    ...
    pub fn new() -> Result<Self, String> {
        ...
        let device_id = match db.get_setting("device_id") {
            Ok(Some(id)) if !id.is_empty() => {
                println!(
                    "[MeshTalk] Loaded persisted device_id: {}",
                    &id[..8.min(id.len())]
                );
                id
            }
            _ => {
                ...
                println!(
                    "[MeshTalk] Generated new device_id: {}",
...
fn dev_log(msg: &str) {
    ...
    if ts.saturating_sub(prev) >= 1 {
        ...
        let log_path = log_dir.join("meshtalk_dev_log.txt");
...
pub fn init_app(state: State<AppState>) -> Result<InitResult, String> {
    ...
    if existing_user.is_none() {
        let hostname = hostname::get()
            ...
            .unwrap_or_else(|_| "MeshTalk User".to_string());
        let user = User {
            ...
            match receiver.recv_timeout(std::time::Duration::from_millis(500)) {
                Ok(msg) => match &msg {
                    SignalingMessage::ChatMessage {
                        ...
                        println!("[MeshTalk] Received chat message from {}", sender_name);
                                        ...
                                        if avatar.starts_with("filemeta:") {
                                            ...
                                            if parts.len() >= 3 {
                                                ...
                                                match db.set_user_avatar(&from, &url) {
                                                    Ok(_) => println!(
                                                        "[MeshTalk] Resolved avatar for {}",
                                                        from
                                                    ),
                                                    Err(e) => println!(
                                                        "[MeshTalk] Failed to resolve avatar: {}",
                                                        e
                                                    ),
                                                }
                        ...
                        match db.create_message(&message) {
                            Ok(_) => println!(
                                "[MeshTalk] Stored incoming message {}",
                                &id[..8.min(id.len())]
                            ),
                            Err(e) => println!("[MeshTalk] Failed to store message: {}", e),
                        }
                    ...
                    SignalingMessage::ProfileUpdate {
                        ...
                        println!("[MeshTalk] Received profile update from {}", from);
                        let _ = db.upsert_peer_as_user(from, username, None);

                        // Resolve avatar URL
                        let resolved_avatar: Option<String> = if let Some(url) = avatar_url {
                            match db.set_user_avatar(from, &url) {
                                Ok(_) => println!("[MeshTalk] Updated avatar for {}", from),
                                Err(e) => println!("[MeshTalk] Failed to set avatar: {}", e),
                            }
                            Some(url.clone())
                            ...
                            if let Some(pc) = signaling.get_peer(&from) {
                                ...
                                match db.set_user_avatar(from, &url) {
                                    Ok(_) => println!("[MeshTalk] Set avatar (file) for {}", from),
                                    Err(e) => println!("[MeshTalk] Failed to set avatar: {}", e),
                                }
                                Some(url)
                            } else {
                                ...
                                match db.set_user_avatar(from, &placeholder) {
                                    Ok(_) => {
                                        println!("[MeshTalk] Stored avatar placeholder for {}", from)
                                    }
                                    Err(e) => println!(
                                        "[MeshTalk] Failed to store avatar placeholder: {}",
                                        e
                                    ),
                                }
                                Some(placeholder)
                    ...
                    SignalingMessage::GroupCreated {
                        ...
                        println!("[MeshTalk] Received group created from {} ({})", from, id);
                        ...
                        match db.create_group(&group) {
                            Ok(_) => println!("[MeshTalk] Stored group {}", &id[..8.min(id.len())]),
                            Err(e) => println!("[MeshTalk] Failed to store group: {}", e),
                        }
                        for (i, uid) in member_ids.iter().enumerate() {
                            ...
                            match db.add_group_member(&gm) {
                                Ok(_) => {}
                                Err(e) => println!("[MeshTalk] Failed to add group member: {}", e),
                            }
                        }
                    ...
                    SignalingMessage::GroupChatMessage {
                        ...
                        println!(
                            "[MeshTalk] Received group chat message from {} in group {}",
                            sender_name,
                        ...
                        match db.send_group_message(&gmsg) {
                            Ok(_) => {
                                println!("[MeshTalk] Stored group message {}", &id[..8.min(id.len())])
                            }
                            Err(e) => println!("[MeshTalk] Failed to store group message: {}", e),
                        }
                    ...
                    SignalingMessage::MeetingChatMessage {
                        ...
                        println!(
                            "[MeshTalk] Received meeting chat from {} (session {})",
                            sender_name,
                    ...
                    SignalingMessage::GroupMemberAdded {
                        ...
                        println!(
                            "[MeshTalk] Group member added: {} to group {}",
                            username,
                            &group_id[..8.min(group_id.len())]
                        );
                        let gm = GroupMember {
                    ...
                    SignalingMessage::GroupMemberRemoved {
                        ...
                        println!(
                            "[MeshTalk] Group member removed: {} from group {}",
                            user_id,
                            &group_id[..8.min(group_id.len())]
                        );
...
/// Download an avatar from remote URL and cache locally in Documents/MeshTalk/avatars/
...
pub fn download_and_cache_avatar(
    ...
    // Create avatars directory: Documents/MeshTalk/avatars/
    ...
    let avatars_path = if cfg!(target_os = "windows") {
        ...
        docs.join("MeshTalk").join("avatars")
    } else if cfg!(target_os = "macos") {
        let home = std::env::var("HOME").unwrap_or_default();
        std::path::PathBuf::from(home).join("Documents/MeshTalk/avatars")
    } else {
        let home = std::env::var("HOME").unwrap_or_default();
        std::path::PathBuf::from(home).join(".local/share/MeshTalk/avatars")
    };
    ...
    match state.db.set_user_avatar(&device_id, &file_url) {
        Ok(_) => println!(
            "[MeshTalk] Cached avatar for {} at {} (served as {})",
            device_id,
            file_path.display(),
            file_url
        ),
        Err(e) => println!("[MeshTalk] Warning: failed to update avatar in DB: {}", e),
    }
...
pub fn register_local_avatar(
    ...
    match state.db.set_user_avatar(&device_id, &local_url) {
        Ok(_) => println!(
            "[MeshTalk] Registered local avatar for {} as {}",
            device_id, local_url
        ),
        Err(e) => println!("[MeshTalk] Warning: failed to update avatar in DB: {}", e),
    }
...
pub fn auto_download_file<R: Runtime>(
    ...
    // Also save to organized downloads: MeshTalk/Downloads/<sender_name>/<type>/<file_name>
...
pub fn rename_user_download_folder(
    ...
    if old_path.exists() && !new_path.exists() {
        ...
        println!(
            "[MeshTalk] Renamed download folder: {:?} -> {:?}",
            old_path, new_path
        );
    }
    Ok(())
...
/// Get the base MeshTalk downloads directory
#[tauri::command]
pub fn get_meshtalk_downloads_base(state: State<AppState>) -> String {
    state
        .file_transfer
        .get_downloads_dir()
        .to_string_lossy()
