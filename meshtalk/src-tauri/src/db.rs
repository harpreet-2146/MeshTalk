// src-tauri/src/db.rs
// SQLite Database Integration for MeshTalk — optimised with WAL, pagination, proper indexing
...
impl Database {
    pub fn get_db_path() -> PathBuf {
        let instance = std::env::var("MESHTALK_INSTANCE")
            .or_else(|_| std::env::var("PINGO_INSTANCE"))
            .unwrap_or_default();

        let base_dir = dirs::data_local_dir().unwrap_or_else(|| PathBuf::from("."));
        let app_name = if instance.is_empty() {
            "MeshTalk".to_string()
        } else {
            format!("MeshTalk_{}", instance)
        };
        let mut app_dir = base_dir.join(&app_name);

        // Backward-compatible fallback to legacy Pingo directory/db if it exists.
        if !app_dir.exists() {
            let legacy_name = if instance.is_empty() {
                "Pingo".to_string()
            } else {
                format!("Pingo_{}", instance)
            };
            let legacy_dir = base_dir.join(&legacy_name);
            let legacy_db = legacy_dir.join("pingo.db");
            if legacy_db.exists() {
                return legacy_db;
            }
