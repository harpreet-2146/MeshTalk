...
export default function ChatPage() {
    ...
    const loadFileDataUrl = useCallback(async (fileId) => {
        ...
        try {
            ...
            console.warn('[MeshTalk] Failed to load file directly:', err);
            return null;
        }
    }, [loadedFileUrls]);
    ...
    const handleImageError = useCallback(async (msg) => {
        try {
            ...
            if (!fileId) {
                console.warn('[MeshTalk] handleImageError: no fileId in message');
                return;
            }
            ...
            try {
                ...
                if (dataUrl) {
                    loadedFileIdsRef.current.add(fileId);
                    setLoadedFileUrls(prev => ({ ...prev, [fileId]: dataUrl }));
                    console.log('[MeshTalk] Loaded file via direct read for', fileId);
                    loadingFilesRef.current.delete(fileId);
                    return;
                }
            } catch (readErr) {
                console.warn('[MeshTalk] Direct read failed for', fileId, readErr);
            }

            // If no sender IP or port, we can't fetch remotely
            if (!senderIp || !port) {
                console.warn('[MeshTalk] handleImageError: no sender IP/port available for', msg.sender_id);
                loadingFilesRef.current.delete(fileId);
                return;
            }
            ...
            console.log('[MeshTalk] Auto-downloading file:', info.fileName, 'from', remoteUrl);
            try {
                ...
                if (dataUrl) {
                    loadedFileIdsRef.current.add(fileId);
                    setLoadedFileUrls(prev => ({ ...prev, [fileId]: dataUrl }));
                    console.log('[MeshTalk] File downloaded and loaded directly');
                }
            } catch (dlErr) {
                console.warn('[MeshTalk] Auto-download failed:', dlErr);
            }

            loadingFilesRef.current.delete(fileId);
        } catch (e) {
            console.error('[MeshTalk] Image error handler failed:', e);
        }
    ...
    const handleDownload = useCallback(async (url, fileName) => {
        try {
            ...
            console.error('[MeshTalk] Save failed:', e);
    ...
    const handleOpenFileLocation = useCallback(async (msg) => {
        try {
            ...
            console.warn('[MeshTalk] Open file location failed:', e);
                    ...
                    try {
                        if (msg.message_type === 'image' || msg.message_type === 'video' || msg.message_type === 'file') {
                            ...
                            if (info && info.fileId) {
                                ...
                                // becomes available and `meshtalk:file-downloaded` will update loadedFileUrls
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                console.warn('[MeshTalk] Failed to load shared media:', e);
            }
        })();
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('meshtalk:open-chat', handleOpenChat);
            return () => {
                window.removeEventListener('meshtalk:open-chat', handleOpenChat);
            };
        }
    ...
    useEffect(() => {
        ...
        window.addEventListener('meshtalk:file-downloaded', handler);
        return () => window.removeEventListener('meshtalk:file-downloaded', handler);
    }, []);
