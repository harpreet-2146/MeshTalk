// src/hooks/useApp.js
// Core application hooks for MeshTalk
...
export function useApp() {
    ...
    const autoDownloadFileMessage = useCallback(async (msg) => {
        ...
        try {
            ...
            if (!senderIp) {
                console.warn('[MeshTalk] Auto-download: no sender IP for', msg.sender_id?.slice(0, 8));
                return;
            }

            const url = `http://${senderIp.split(':')[0]}:${info.port}/file/${info.fileId}`;
            console.log('[MeshTalk] Auto-downloading file:', info.fileName, 'from', url);
                ...
                .then(async () => {
                    ...
                    try {
                        ...
                        if (dataUrl && typeof window !== 'undefined') {
                            window.dispatchEvent(new CustomEvent('meshtalk:file-downloaded', {
                                detail: { fileId: info.fileId, dataUrl }
                            }));
                        }
                    } catch { /* ignore — UI will lazy-load via effect */ }
                })
                .catch(e => console.warn('[MeshTalk] Auto-download failed:', e));
    ...
    useEffect(() => {
        ...
        const wb = setTimeout(() => {
            if (!finished) {
                console.warn('[MeshTalk] bootstrap still not complete after 10s');
                api.appendDevLog && api.appendDevLog('[MeshTalk] bootstrap still not complete after 10s').catch(() => { });
            }
        }, 10000);

        (async () => {
            try {
                console.log('[MeshTalk] bootstrap started');
                api.appendDevLog && api.appendDevLog('[MeshTalk] bootstrap started').catch(() => { });

                const init = await api.initApp();
                api.appendDevLog && api.appendDevLog('[MeshTalk] initApp returned').catch(() => { });
                if (cancelled) return;
                ...
                api.appendDevLog && api.appendDevLog('[MeshTalk] startSignaling returned').catch(() => { });

                // Start discovery
                const username = user?.username || 'MeshTalk User';
                await api.startDiscovery(username, 45678);
                api.appendDevLog && api.appendDevLog('[MeshTalk] startDiscovery returned').catch(() => { });
                ...
                try {
                    ...
                    api.appendDevLog && api.appendDevLog('[MeshTalk] file server port=' + (port || 0)).catch(() => { });
                } catch { /* ignore */ }
                ...
                finished = true;
                clearTimeout(wb);
                api.appendDevLog && api.appendDevLog('[MeshTalk] bootstrap complete').catch(() => { });
            } catch (err) {
                console.error('[MeshTalk] Init failed:', err);
                api.appendDevLog && api.appendDevLog('[MeshTalk] init failed: ' + String(err)).catch(() => { });
                setError(String(err));
            }
        })();
    ...
    const flushPendingMessages = useCallback(async (peerDeviceId) => {
        ...
        if (now - lastFlush < 30000) {
            console.debug('[MeshTalk] Skipping flush for', peerDeviceId.slice(0, 8), '- rate limited');
            return;
        }

        // Prevent concurrent flushes for same peer
        if (flushInProgressRef.current.has(peerDeviceId)) {
            console.debug('[MeshTalk] Skipping flush for', peerDeviceId.slice(0, 8), '- already in progress');
            return;
        }
        ...
        try {
            ...
            for (const p of toSend) {
                try {
                    ...
                    console.warn('[MeshTalk] Failed to deliver pending message', p.id, err);
                }
            }
            ...
            console.warn('[MeshTalk] flushPendingMessages error:', err);
        } finally {
            flushInProgressRef.current.delete(peerDeviceId);
        }
    }, [refreshLastMessages]);
    ...
    useEffect(() => {
        ...
        unsubs.push(api.onPeerDiscovered(async peer => {
            ...
            console.debug('[MeshTalk][debug] onPeerDiscovered — id=%s incomingUsername=%s existingUsername=%s', peer.device_id, peer.username, existingUser?.username);
        ...
        unsubs.push(api.onPeerUpdated(peer => {
            ...
            const existingUser = peerMapRef.current.get(peer.device_id);
            console.debug('[MeshTalk][debug] onPeerUpdated — id=%s incomingUsername=%s existingUsername=%s ip=%s port=%s', peer.device_id, peer.username, existingUser?.username, ip, port);
            ...
            if (peer.username && existingUser?.username && peer.username !== existingUser.username) {
                console.debug('[MeshTalk][debug] onPeerUpdated: suppressed presence username update — incoming=%s existing=%s id=%s', peer.username, existingUser.username, peer.device_id);
            }
            ...
            if (!resolvedUsername && peer.username) {
                if (!prevPresenceLocked) {
                    ...
                    console.debug('[MeshTalk][debug] onPeerUpdated: accepted presence-derived username=%s for id=%s', resolvedUsername, peer.device_id);
                } else {
                    // Already locked to a presence-derived username — ignore changes
                    if (prev.username && prev.username !== peer.username) {
                        console.debug('[MeshTalk][debug] onPeerUpdated: ignored changed presence username (locked) — incoming=%s locked=%s id=%s', peer.username, prev.username, peer.device_id);
                    }
                }
            }
        ...
        unsubs.push(api.onSignalingMessage(msg => {
            ...
            if (msg?.type === 'DeliveryAck' && msg?.from && msg?.message_id) {
                ...
                try {
                    if (typeof window !== 'undefined') {
                        window.dispatchEvent(new CustomEvent('meshtalk:pending-delivered', { detail: { peerId, messageId } }));
                    }
                } catch (e) { /* ignore */ }
            ...
            if (msg?.type === 'ProfileUpdate' && msg?.from) {
                ...
                console.debug('[MeshTalk][debug] ProfileUpdate received — peerId=%s msg.username=%s existingUsername=%s avatar=%s', peerId, msg.username, existingUser?.username, msg.avatar_url ? 'yes' : 'no');
                ...
                console.debug('[MeshTalk][debug] ProfileUpdate applied — peerId=%s newUsername=%s avatar_path=%s', peerId, updated.username, updated.avatar_path ? 'set' : 'none');
                setPeers(Array.from(peerMapRef.current.values()));
            ...
            if (user.avatar_path.startsWith('http://') || user.avatar_path.startsWith('https://')) {
                ...
                tasks.push((async () => {
                    try {
                        ...
                        console.warn(`[MeshTalk] Failed to cache avatar for ${user.username}:`, err);
                    }
                })());
            }
        }
        await Promise.all(tasks).catch(err => console.warn('[MeshTalk] Avatar caching batch failed:', err));
    ...
    useEffect(() => {
        const handleNotificationClick = (event) => {
            ...
            console.log('[MeshTalk] Notification clicked - opening chat with', senderName, '(', senderId, ')');

            // Dispatch event for chat page to handle navigation
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('meshtalk:open-chat', {
        ...
        const handleWindowFocus = () => {
            ...
            if (lastNotif && lastNotif.senderId) {
                console.log('[MeshTalk] App focused - checking for pending notifications');
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('meshtalk:notification-clicked', handleNotificationClick);
            window.addEventListener('focus', handleWindowFocus);
            return () => {
                window.removeEventListener('meshtalk:notification-clicked', handleNotificationClick);
    ...
    const saveAvatar = useCallback(async (imageData) => {
        ...
        try {
            ...
            console.warn('[MeshTalk] Avatar broadcast failed:', e);
        }
        return result;
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && import.meta.env.DEV) {
            ...
            window.meshtalkFlushPending = (peerId) => flushPendingMessages(peerId).catch(e => console.warn('[MeshTalk] meshtalkFlushPending failed', e));
        }
        return () => { if (typeof window !== 'undefined') delete window.meshtalkFlushPending; };
...
export function useChat() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('meshtalk:pending-delivered', handler);
        return () => window.removeEventListener('meshtalk:pending-delivered', handler);
    }, []); // Empty deps — uses ref
