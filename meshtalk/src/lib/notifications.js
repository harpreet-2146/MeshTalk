...
// Native Windows Notifications for MeshTalk with Avatar Support
...
async function showNotification(body, options = {}) {
    ...
    if (!isTauri) {
        console.log(`[📬 Notification from MeshTalk] ${body}`);
        if ('Notification' in window && Notification.permission === 'granted') {
            const notification = new Notification('MeshTalk Messenger', { body, ...options });
            if (options.onClick) {
    ...
    try {
        ...
        // Send native Windows notification with "MeshTalk Messenger" title
        // Format: "MeshTalk Messenger" with body like "John Doe: Hey bro are you free?"
        await sendNotification({
            title: 'MeshTalk Messenger',
            body: body,
            tag: notificationId
        });
 ...
 * Displays as: "MeshTalk Messenger" | "John Doe: Hey bro are you free?"
