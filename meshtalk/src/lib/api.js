...
async function getEventApi() {
    if (!eventPromise) {
        eventPromise = import('@tauri-apps/api/event').catch(() => null);
    }
    return eventPromise;
}
...
async function isTauriRuntime() {
    ...
    return !!(core && typeof core.isTauri === 'function' && core.isTauri());
}
...
export const getMeshTalkDownloadsBase = () => invoke('get_meshtalk_downloads_base');
